...
router.use(checkCrudPermissions('transactions'));
...
router.get(
  ...
  wrapAsync(async (req, res) => {
    ...
    const payload = await TransactionsDBApi.findAll(req.query, globalAccess, {
      currentUser,
    });
...
router.get(
  '/:id',
  wrapAsync(async (req, res) => {
    const payload = await TransactionsDBApi.findBy({ id: req.params.id });
